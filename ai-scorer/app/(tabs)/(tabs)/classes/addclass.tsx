...
export default function AddClass() {
  ...
  const [theme, setTheme] = useState<string | null>(SWATCHES[0]);
  const [themeModal, setThemeModal] = useState(false);
  const [yearOpen, setYearOpen] = useState(false);

  function onCreate() {
    // TODO: hook to store the class in context/server
    if (!name || !section) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#fff" }}>
      <LinearGradient colors={["#00b679", "#009e60"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.header}>
